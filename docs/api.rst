...
API will be kept backwards compatible for at least the next release, even if
...
.. py:class:: botogram.Bot(api_connection)
   ...
   .. py:decoratormethod:: message_contains(string, [ignore_case=True, multiple=False])
      ...
      called when multiple matches are found in the same message. Decorated
      functions will be called with two parameters:
